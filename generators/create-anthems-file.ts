...
/**
 * MIDI is a synthesised transcription, not a recording — it sounds nothing like
 * the anthem a player would recognise, and a `.wav` rendered from one is just
 * as tinny, so the WORD matters as much as the extension.
 *
 * Historical and politically superseded takes are excluded too: a "former"
 * anthem is the wrong answer to "whose anthem is this", and a Francoist-era
 ...
 *
 * Search relevance alone is NOT evidence: querying an ISO code returns whatever
 * anthem recording ranks highest, which is how Afghanistan first shipped the
 * US Air Force Band playing the Star-Spangled Banner. A hit only counts when
 * its FILENAME independently corroborates the anthem's title or the country's
 * name — and never when it names some other country outright.
 */
const corroborates = (file: string, anthem: string, country: string): boolean => {
  ...
  if (!wanted.some(word => haystack.some(found => found.startsWith(stem(word)))))
    return false
  ...
  const withoutPerformer = file.replace(PERFORMER_CREDIT, ' ')
  const foreign = OTHER_COUNTRY_WORDS.filter(
    word => significantWords(withoutPerformer).includes(word) && !ours.has(word)
  )
  return foreign.length === 0
}

/** Every country's name words, for the "names someone else" check above. */
