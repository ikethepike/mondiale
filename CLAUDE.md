...
| Challenge view chrome (full-viewport shell, footer berth + safe area, guess-box, lock-row) | `.challenge-shell` in `templates/_challenge-shell.scss` — the passthrough shell is `pointer-events: none`; shared strips (guess-box, lock-row, chip lists, option cards, hint chips) opt back in inside their templates, and self-contained components opt themselves in. A view-level `pointer-events: auto` is only for bespoke positioned stages |
...
| Typed guess with live suggestions | countries: `components/country/CountryGuessInput.vue`; every other register (water features, empires…): `components/challenge/SuggestInput.vue` — consoles restyle both through `.guess-form`/`.suggestions`; views never inline their own dropdown |
